...
this.settings=JSON.parse(args.settings);this.bar_elm=jQuery(this.settings.notify_div_id);this.showagain_elm=jQuery(this.settings.showagain_div_id);this.main_button=jQuery('.cli-plugin-main-button');this.main_link=jQuery('.cli-plugin-main-link');this.reject_link=jQuery('.cookie_action_close_header_reject');this.delete_link=jQuery(".cookielawinfo-cookie-delete");if(this.settings.as_popup){CLI_COOKIEBAR_AS_POPUP=!0}
this.configShowAgain();this.configBar();this.attachStyles();this.toggleBar();this.attachDelete();this.attachEvents();this.configButtons();if(this.settings.scroll_close===!0){window.addEventListener("scroll",CLI.closeOnScroll,!1)}},attachEvents:function(){jQuery('.cli_action_button').click(function(e){e.preventDefault();var elm=jQuery(this);var button_action=elm.attr('data-cli_action');var open_link=elm[0].hasAttribute("href")&&elm.attr("href")!='#'?!0:!1;var new_window=!1;if(button_action=='accept'){CLI.accept_close();new_window=CLI.settings.button_1_new_win?!0:!1}else if(button_action=='reject'){CLI.reject_close();new_window=CLI.settings.button_3_new_win?!0:!1}
...
this.reject_link.css('color',this.settings.button_3_link_colour);if(this.settings.button_3_as_button){this.reject_link.css('background-color',this.settings.button_3_button_colour);this.reject_link.hover(function(){jQuery(this).css('background-color',CLI.settings.button_3_button_hover)},function(){jQuery(this).css('background-color',CLI.settings.button_3_button_colour)})}},toggleBar:function(){if(CLI_COOKIEBAR_AS_POPUP){this.barAsPopUp()}
if(!CLI_Cookie.exists(CLI_ACCEPT_COOKIE_NAME)){this.displayHeader()}else{this.bar_elm.hide()}
...
if(this.settings.notify_position_horizontal=="left"){this.showagain_config.left=this.settings.showagain_x_position}else if(this.settings.notify_position_horizontal=="right"){this.showagain_config.right=this.settings.showagain_x_position}},configBar:function(){this.bar_config={'background-color':this.settings.background,'color':this.settings.text,'font-family':this.settings.font_family};if(this.settings.notify_position_vertical=="top"){this.bar_config.top='0';if(this.settings.header_fix===!0){this.bar_config.position='fixed'}}else{this.bar_config.bottom='0'}
if(this.settings.notify_position_vertical=="top"){if(this.settings.border_on){this.bar_config['border-bottom']='2px solid '+this.l1hs(this.settings.border)}
this.showagain_config.top='0'}else if(this.settings.notify_position_vertical=="bottom"){if(this.settings.border_on){this.bar_config['border-top']='2px solid '+this.l1hs(this.settings.border)}
this.bar_config.position='fixed';this.bar_config.bottom='0';this.showagain_config.bottom='0'}},l1hs:function(str){if(str.charAt(0)=="#"){str=str.substring(1,str.length)}else{return"#"+str}
...
this.showagain_elm.slideDown(this.settings.animate_speed_show);if(this.settings.accept_close_reload===!0){this.reload_current_page()}
...
this.showagain_elm.slideDown(this.settings.animate_speed_show);if(this.settings.reject_close_reload===!0){this.reload_current_page()}
...
this.showagain_elm.hide();if(CLI_COOKIEBAR_AS_POPUP){this.showPopupOverlay()}},hideHeader:function(){if(this.settings.notify_animate_show){this.showagain_elm.slideDown(this.settings.animate_speed_show)}else{this.showagain_elm.show()}
this.bar_elm.slideUp(this.settings.animate_speed_show);this.hidePopupOverlay()},hidePopupOverlay:function(){jQuery('body').removeClass("cli-barmodal-open");jQuery(".cli-popupbar-overlay").removeClass("cli-show")},showPopupOverlay:function(){if(this.settings.popup_overlay){jQuery('body').addClass("cli-barmodal-open");jQuery(".cli-popupbar-overlay").addClass("cli-show")}},barAsPopUp:function(){if(typeof cookie_law_info_bar_as_popup==='function'){return!1}
